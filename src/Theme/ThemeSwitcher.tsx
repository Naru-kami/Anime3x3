...
const MaterialUISwitch = styled(Switch)(({ theme }) => ({
  ...
  '& .MuiSwitch-switchBase': {
    ...
    '&.Mui-checked': {
      ...
      '& + .MuiSwitch-track': {
        opacity: 1,
        backgroundColor: theme.palette.mode === 'dark' ? '#535b65' : '#aab4be',
      },
    },
  },
  '& .MuiSwitch-thumb': {
...
export default function ThemeSwitcher() {
  const [theme, setStore] = useStore(store => store.darkTheme);
  const handleTheme = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    setStore({ ['darkTheme']: e.target.checked })
  }, [setStore]);
  return <MaterialUISwitch checked={theme} onChange={handleTheme} />
};